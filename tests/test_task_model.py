...
def test_task_aware_head_learns_different_model_preferences() -> None:
    ...
    parser_goal = GoalContext("parser json wrapper", "heldout", ("cheap", "strong"))
    theorem_goal = GoalContext("symbolic theorem", "heldout", ("cheap", "strong"))

    assert (
        head.estimate(parser_goal, RouterState(), "cheap").success_probability
        > head.estimate(parser_goal, RouterState(), "strong").success_probability
    )
    assert (
        head.estimate(theorem_goal, RouterState(), "strong").success_probability
        > head.estimate(theorem_goal, RouterState(), "cheap").success_probability
    )
