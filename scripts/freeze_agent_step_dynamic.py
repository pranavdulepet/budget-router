#!/usr/bin/env python3
"""Freeze every input that can affect paid agent-step execution."""
...
def main() -> None:
    ...
    paths = [
        ...
        Path("artifacts/active_router_task_manifest.json"),
        ...
        Path("outputs/agent_step_router_v1/split_manifest.json"),
        Path("outputs/agent_step_router_v1/router_artifact.json"),
        Path("outputs/agent_step_router_v1/training_report.json"),
        Path("outputs/agent_step_router_v1/static_gate.json"),
    ...
    if missing:
        raise SystemExit("missing dynamic inputs: " + ", ".join(missing))
    ...
    static_gate = _load(Path("outputs/agent_step_router_v1/static_gate.json"))
    if static_gate.get("dynamic_collection_authorized") is not True:
        raise PermissionError("static gate did not authorize a dynamic freeze")
    artifact = _load(Path("outputs/agent_step_router_v1/router_artifact.json"))
    protocol = _load(
        Path(
            "artifacts/active_router_protocol_amendment_007_agent_step_routing.json"
        )
