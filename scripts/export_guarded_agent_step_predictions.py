#!/usr/bin/env python3
"""Export a complete Amendment 008 stage for official SWE-bench grading."""
...
try:
    ...
    from run_agent_step_study import FIXED_CHEAP, FIXED_STRONG  # type: ignore[no-redef]
    from run_guarded_agent_step_followup import (  # type: ignore[no-redef]
        HELDOUT_POLICIES,
        ROUTED,
    )
...
def export_predictions(
    *,
    ...
    if stage not in {"development", "heldout"}:
        raise ValueError("stage must be development or heldout")
    protocol = _load(protocol_path)
    manifest = _load(task_manifest_path)
    artifact = GuardedAgentStepArtifact.from_dict(_load(artifact_path))
    task_ids = [str(row["task_id"]) for row in manifest[stage]]
    policies = (ROUTED,) if stage == "development" else HELDOUT_POLICIES
    records = [
        row
    ...
    for row in records:
        ...
        if key in by_key:
            raise ValueError(f"duplicate guarded stage record: {key}")
    ...
    if missing or unexpected:
        raise ValueError(
            f"guarded {stage} matrix mismatch: missing={missing}, "
    ...
    for policy_id in policies:
        ...
        for row in policy_records:
            ...
            if not patch:
                raise ValueError(
                    f"{policy_id} {task_id} is submitted but patch is absent"
                )
    ...
    result = {
        "schema_version": "guarded-agent-step-predictions-v2",
...
def main() -> None:
    ...
    output_dir = args.output_dir or (
        Path("outputs/agent_step_router_v2/swebench_grader")
        / args.stage
        / "predictions"
    )
